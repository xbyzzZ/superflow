...
六类专业子代理不是固定串行链路。Superflow 根据任务条件路由：纯后端缺陷不需要 UI 设计师，跨模块页面功能则可能需要架构、UI、前端、后端、测试和审查共同参与。
...
- CodeGraph MCP 可用时，代码探索优先使用 CodeGraph。
...
~~~bash
unzip superflow-v0.1.0.zip -d ~/.codex/skills
~~~
